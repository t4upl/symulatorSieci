...
public class OptimizerEV extends OptimizerCore {
	...
	void dodajOgraniczeniaHandlu() throws IloException
	{
		...
		{
			//getInput("dodajOgraniczeniaHandlu() not null");
			...
			/*if (form.getIDprosumenta()==3)
			{
				print(handelOgraniczeniaDomu.getKupuj());
				print(handelOgraniczeniaDomu.getGeneration());
				print(handelOgraniczeniaDomu.getConsumption());
				
				print("");
				print(handelOgraniczeniaEV.getKupuj());
				print(handelOgraniczeniaEV.getGeneration());
				print(handelOgraniczeniaEV.getConsumption());
				getInput("dodajOgraniczeniaHandlu -debug stop");
			}*/
	...
	{	
		...
		while(i<Stale.horyzontCzasowy)
		{
			...
			case 1: ograniczeniaStatusuDomPraca(i); break;
			case 2:	ograniczeniaStatusuPraca(i); break;
			case 3:	ograniczeniaStatusuPracaDom(i); break;
	...
	void fillOutSolvedVariablesDayDataLoop(Sterowanie sterowanie)
	{
		...
		while (i<Stale.horyzontCzasowy)
		{
			...
			if (i+1<Stale.horyzontCzasowy)
			{
				d.setStanBateriiNaKoniecSlotu((float)EB_solved[i+1]);
			}
			
			d.setZBateriiNaKonsumpcje((float)EB_c_solved[i]);
			d.setZBateriiNaRynek((float)EB_s_solved[i]);
			
			d.setZGeneracjiNaKonsumpcje((float)G_c_solved[i]);
			d.setZGeneracjiDoBaterii((float)G_sklad_solved[i]);
			d.setZGeneracjiNaRynek((float)G_s_solved[i]);
			
			d.setZRynekDoBaterii((float)EM_s_solved[i]);
			d.setZRynekNaKonsumpcje((float)EM_c_solved[i]);
			
			d.setCenaNaLokalnymRynku(form.getPrices().get(i));
			d.setKupuj((float)binHandelKupuj_solved[i]);
	...
	void fillOutSolvedVariablesEVDataLoop(Sterowanie sterowanie)
	{
		...
		while (i<Stale.horyzontCzasowy)
		{
			...
			ev.setKoszt(koszt_solved[i]);
			ev.setKoszt_Zew(koszt_Zew_solved[i]);
			ev.setKoszt_sklad(koszt_sklad_solved[i]);
			ev.setKoszt_EV(koszt_EV_solved[i]);
			ev.setKoszt_handel(koszt_handel_solved[i]);
			
			ev.setEV((float)EV_solved[i]);
			ev.setEV_c((float)EV_c_solved[i]);
			ev.setEV_EB((float)EV_EB_solved[i]);
			ev.setEB_EV((float)EB_EV_solved[i]);
			
			ev.setG_EV((float)G_EV_solved[i]);
			ev.setZew_EV((float)Zew_EV_solved[i]);
			
			ev.setEM_EV((float)binEVKupuj_solved[i]*(float)EM_EV_solved[i]);
			ev.setEV_EM((1-(float)binEVKupuj_solved[i])*(float)EV_EM_solved[i]);
