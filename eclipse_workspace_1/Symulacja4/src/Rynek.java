...
public class Rynek extends CoreClass {
   ...
   //dla scenariusz bez EV = 16 (jedno na prosuemnta)
	...
	public void addPricePointEV(Prosument prosument, Point point)
	{
		
		int idListy = prosument.getID()+Stale.liczbaProsumentow;
		ArrayList<Point> pointList = listaFunkcjiUzytecznosci.get(idListy);		
	...
	void rozpuscKontraktyWsrodProsumentow(ArrayList<DayData> kontraktyDlaKont)
	{
		...
		while (i<Stale.liczbaProsumentow)
		{
			...
			if (Stale.isScenariuszEV && prosument instanceof ProsumentEV )
			{
				((ProsumentEV)prosument).getKontrakt(priceVector,kontraktyDlaKont.get(i),kontraktyDlaKont.get(i+Stale.liczbaProsumentow));
			}else
			{
				prosument.getKontrakt(priceVector,kontraktyDlaKont.get(i));
